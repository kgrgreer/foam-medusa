...
foam.CLASS({
  ...
  name: 'CompactionDAO',
  extends: 'foam.core.medusa.BlockingDAO',
  ...
  methods: [
    {
      ...
      if ( COMPACTION_CMD.equals(obj) ) {
        foam.core.script.ScriptParameter sp = (foam.core.script.ScriptParameter) ((DAO) x.get("scriptParameterDAO")).find("compactionDAO");
        if ( sp != null ) {
    ...
    },
    {
      ...
      args: 'X x',
      javaCode: `
      COREService service = (COREService) x.get("promotedClearAgent");
      if ( service != null ) {
        service.stop();
      }
      service = (COREService) x.get("medusaConsensusMonitor");
      if ( service != null ) {
        service.stop();
      }
      `
  ...
  classes: [
    {
      ...
      documentation: 'Delegates once per CSpec-ObjectId',
      ...
      methods: [
        {
          ...
          String id = entry.getCSpecName()+"-"+entry.getObjectId();
    ...
    },
    {
      ...
      javaCode: `
        public CSpecSink(X x, CompactionDAO self, ProxySink delegate) {
          super(x, delegate);
          setSelf(self);
        }
      `,
      ...
      properties: [
        {
          ...
          of: 'foam.core.medusa.CompactionDAO'
        }
      ],
      ...
      methods: [
        {
          ...
          if ( entry.getObjectId() == null ) {
             if ( ! "bootstrap".equals(entry.getCSpecName()) ) {
               logger.error("Object id null", entry.getCSpecName(), entry.getDop(), entry.getObjectId());
             }
             return;
          }
          ...
          DAO mdao = null;
          try {
            mdao = (DAO) support.getMdao(x, entry.getCSpecName());
          ...
          if ( found == null ) {
            if ( entry.getDop().equals(DOP.REMOVE) ) {
              // OK
              Map map = (Map) getSelf().getNotFound().get(entry.getCSpecName());
              if ( map != null ) {
                map.remove(entry.getObjectId());
              }
              logger.info("Object removed", entry.getCSpecName(), entry.getDop(), entry.toSummary());
            } else {
              Map map = (Map) getSelf().getNotFound().get(entry.getCSpecName());
              if ( map == null ) {
                map = new HashMap();
                getSelf().getNotFound().put(entry.getCSpecName(), map);
              }
              if ( ! map.containsKey(entry.getObjectId()) ) {
                map.put(entry.getObjectId(), entry);
              }
              // delay reporting not found, see 'report'
              // logger.error("Object not found", entry.getCSpecName(), entry.getDop(), entry.toSummary());
            }
          } else {
            Compaction compaction = (Compaction) ((DAO) x.get("compactionDAO")).find(entry.getCSpecName());
            if ( found instanceof LifecycleAware &&
              ...
              logger.info("Object removed, LifecycleState.DELETED", entry.getCSpecName(), entry.getDop(), entry.toSummary());
            } else {
