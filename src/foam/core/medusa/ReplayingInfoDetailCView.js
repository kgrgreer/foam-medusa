...
//       return;
      ...
      label.text$ = this.health$.map(function(h) {
        return 'Uptime: '+foam.lang.Duration.duration(h.uptime);
      });
      this.add(label);
      ...
      if ( this.config.replayingInfo.replaying ) {
        ...
        label.text$ = this.config$.map(function(c) {
          return 'Elapsed: '+foam.lang.Duration.duration(c.replayingInfo.elapsedTime);
        });
        this.add(label);
        ...
        label = this.makeLabel();
        label.text$ = this.config$.map(function(c) { return 'Remaining: '+foam.lang.Duration.duration(c.replayingInfo.timeRemaining); });
        this.add(label);
      ...
      label.text$ = this.config$.map(function(c) {
        ...
        return 'Idle: '+foam.lang.Duration.duration(delta);
      });
      this.add(label);
